...
OBJECTS = {"dobj", "dative", "attr", "oprd","iobj","conj","pobj"}
...
def _get_all_objs(v, is_pas):
    ...
    if len(objs) == 0:
    	for tok_ in rights: 
    		objs = [tok for tok in tok_.rights if tok.dep_ in OBJECTS or (is_pas and tok.dep_ == 'pobj')]
    objs.extend(_get_objs_from_prepositions(rights, is_pas))
            ...
            if isConjVerb:
                    ...
                    if(len(objs) == 0):
                        if is_pas:  # reverse object / subject for passive
                            ...
                            svos.append((to_str(expand(sub, tokens)),
                                         "!" + v.lower_ if verbNegated or objNegated else v.lower_, to_str(expand(obj, tokens))))
                            svos.append((to_str(expand(sub, tokens)),
                                         "!" + v2.lower_ if verbNegated else v2.lower_, to_str(expand(obj, tokens))))
            else:
                ...
                for sub in subs:
                    ...
                    if(len(objs) == 0):
                        print("hel")
                        if is_pas:  # reverse object / subject for passive
                            svos.append(v.lemma_ if verbNegated else v.lemma_ + " v->o " + to_str(expand(sub, tokens)))
                        else:
                           svos.append((to_str(expand(sub, tokens)),
                ...
                if(len(objs > 0)):
                    for obj in objs:
                        ...
                        if is_pas:  # reverse object / subject for passive
                            ...
                            svos.append((to_str(expand(obj, tokens)),
                                         "!" + v2.lemma_ if verbNegated else v2.lemma_))
                        else:
                            svos.append("!" + v.lower_ if verbNegated else v.lower_ +" v->o "+ to_str(expand(obj, tokens)))
                            svos.append("!" + v2.lower_ if verbNegated else v2.lower_ +" v->o "+ to_str(expand(obj, tokens)))
                else:
                    if is_pas:  # reverse object / subject for passive
                        svos.append("!" + v.lemma_ if verbNegated else v.lemma_)
                                         
                        svos.append("!" + v2.lemma_ if verbNegated else v.lemma_)
                    else:
                        svos.append("!" + v.lemma_ if verbNegated else v.lemma_)
                        svos.append("!" + v2.lemma_ if verbNegated else v.lemma_)
            else:
                ...
                if(len(objs) > 0):
                    ...
                    for obj in objs:
                        ...
                        if is_pas:  # reverse object / subject for passive
                            ...
                            svos.append((
                                         "!" + v.lower_ if verbNegated or objNegated else v.lower_+" v->o "+to_str(expand(obj, tokens))))
                    ...
                    if is_pas:  # reverse object / subject for passive
                        ...
                        svos.append("!" + v.lower_ if verbNegated or objNegated else v.lower_+ " v->o " +to_str(expand(obj, tokens)))
...
txt = ""
txt = "suresh and ram are playing cricket"
tokens = nlp(txt)
svos = findSVOs(tokens)
print(txt)
printDeps(tokens)
print(svos)
spacy.displacy.serve(tokens, style='dep', port=5001) # http://localhost:5000/
